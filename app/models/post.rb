class Post < ApplicationRecord
  validates :weight, presence: true, numericality: true, { greater_than_or_equal_to: 30.0, less_than_or_equal_to: 300.0, message: 'は半角数値の30.0kg〜300.0kgの範囲内で入力してください' }, format: {with: /\A[0-9]+(\.[0-9]+)?\z/
  }
  validates :fat_percentage, numericality: true, { greater_than_or_equal_to: 3.0, less_than_or_equal_to: 99.9, message: 'は半角数値の3.0%〜99.9%の範囲内で入力してください' }, format: {with: /\A[0-9]+(\.[0-9]+)?\z/
  }
